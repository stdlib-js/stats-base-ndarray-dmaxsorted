...
var dmaxsorted = tryRequire( resolve( __dirname, './../lib/native.js' ) );
var opts = {
...
tape( 'the function has an arity of 1', opts, function test( t ) {
	t.strictEqual( dmaxsorted.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the maximum value of a one-dimensional ndarray', opts, function test( t ) {
	...
	v = dmaxsorted( [ vector( x, 2, 1, 0 ) ] );
	t.strictEqual( v, -4.0, 'returns expected value' );

	x = new Float64Array( [ -0.0, -0.0, 0.0 ] );
	v = dmaxsorted( [ vector( x, 3, 1, 0 ) ] );
